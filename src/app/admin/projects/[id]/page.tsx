...
export default function EditProjectPage() {
    ...
    (async () => {
      try {
        ...
        if (Array.isArray(data.showcasePhotos) && data.showcasePhotos.length > 0) {
          setPhotos(data.showcasePhotos.map((url: string) => ({ url, file: null })));
        }
        // Contributors
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const showcasePhotos: string[] = photos.map((p: { url: string }) => p.url).filter(Boolean);
      const body = {
        title,
        shortDescription,
        description,
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            {posterUrl && (
              <Image src={posterUrl} alt="Poster" className="w-12 h-12 object-cover rounded border" width={48} height={48} />
            )}
            {posterUrl && (
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            {thumbnailUrl && (
              <Image src={thumbnailUrl} alt="Thumbnail" className="w-12 h-12 object-cover rounded border" width={48} height={48} />
            )}
            {thumbnailUrl && (
          ...
          <ul className="space-y-2 mb-2">
            {photos.map((p, idx) => (
              <li key={idx} className="flex gap-2 items-center">
                ...
                <input
                  ...
                  onChange={async e => {
                    ...
                    try {
                      ...
                      setPhotos(prev => prev.map((p, i) => i === idx ? { ...p, uploading: false, error: err instanceof Error ? err.message : "Upload failed" } : p));
                    }
                  }}
                ...
                {p.url && !p.uploading && (
                  <Image src={p.url} alt="Showcase" className="w-12 h-12 object-cover rounded border" width={48} height={48} />
                )}
                {p.url && !p.uploading && (
