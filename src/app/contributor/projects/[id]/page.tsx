...
export default function EditProjectPage() {
  ...
  // Check authorization: verify contributor is associated with this project
  useEffect(() => {
    if (!session?.user?.email || !id) return;
    
    (async () => {
      try {
        // Get contributor ID from email
        const contributorRes = await fetch(`/api/contributors?email=${encodeURIComponent(session.user.email!)}`);
        if (!contributorRes.ok) {
          setError("You are not registered as a contributor");
          setLoading(false);
          return;
        }
        
        const contributorData = await contributorRes.json();
        const contributor = Array.isArray(contributorData.contributors) ? contributorData.contributors[0] : null;
        ...
        setCurrentContributorId(contributor._id);
        
        // Check if this contributor is associated with the project
        ...
        const isAssociated = Array.isArray(projectData.contributors) && 
          projectData.contributors.some((c: { id: string }) => c.id === contributor._id);
        
        if (!isAssociated) {
          setError("You are not authorized to edit this project. You can only edit projects you are associated with.");
          setLoading(false);
          return;
        }
        
        // Contributor is authorized
